...
describe('PHP "ob_clean" builtin function', function () {
    ...
    beforeEach(function () {
        ...
        ob_clean = state.getFunction('ob_clean');
    });

    it('should clean the current output buffer', async function () {
        await ob_clean.call([]).toPromise();

        expect(output.cleanCurrentBuffer).to.have.been.calledOnce;
    });

    describe('on success', function () {
        it('should return true', async function () {
            var resultValue = await ob_clean.call([]).toPromise();
    ...
    describe('on failure', function () {
        beforeEach(function () {
            output.cleanCurrentBuffer.throws(new NoActiveOutputBufferException());
        });
